...
#include "s5k4h5ycmipiraw2lane_Sensor.h"
...
{ 
	...
	write_cmos_sensor(0x0202, 0x00);//04
	write_cmos_sensor(0x0203, 0x20);//c0
	...
	write_cmos_sensor(0x0345, 0x08);
		...
		write_cmos_sensor(0x0349,0xC9);//Caval 140613
...
{              LOG_INF("E//VGA 120fps");
			...
			write_cmos_sensor(0x0114,0x01);       //MIPI Lains	Mipi lanes                                            
			                  
			write_cmos_sensor(0x0301,0x04);       //PLL control	vt_pix_clk_div                                      
